from .types import (MidiMessageType, NoteOff, NoteOn, PolyphonicAftertouch,
...
TYPES_ATTRIBUTES: dict[type[MidiMessageType], list[str]] = {
...
class MessageBuilder:
    ...
    def __init__(self, status, data1, data2):
        assert 128 <= status <= 255, f'Bad status value. ({status})'
        self.status = status
        self.data1 = data1
        self.data2 = data2
        self._message = None
    ...
    def _build(self):
        ...
        if self.data2 is not None:
            message_type = midi_type(self.data1, self.data2, _internal=True)
        else:
            message_type = midi_type(self.data1, _internal=True)
